// src/modules/BugStatus/bugStatusParser.ts
// Parser for Bug Status - preserves exact Excel field names
...
export async function parseBugStatusFile(file: File): Promise<BugStatusAnalytics> {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const statusColIdx = findColumnIndex(headers, BUG_STATUS_COLUMN_ALIASES)
        ...
        const allRecords: BugStatusRecord[] = []
        const dataRows = rows.slice(1).filter(row => row && row.length > 0)

        for (const row of dataRows) {
          const record: BugStatusRecord = {}
