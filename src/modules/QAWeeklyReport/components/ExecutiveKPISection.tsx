...
const KPICard: React.FC<KPICardProps> = ({
  ...
  // 3D transform values - subtle but premium
    ...
    >
      <motion.div
        ...
        style={{
          ...
          boxShadow: isHovered
            ? theme === 'dark'
              ? '0 25px 50px -12px rgba(0,0,0,0.6), 0 0 0 1px rgba(212,175,55,0.15)'
              : '0 25px 50px -12px rgba(0,0,0,0.15), 0 0 0 1px rgba(212,175,55,0.2)'
            : theme === 'dark'
      ...
      >
        ...
        <div
          ...
          style={{
            background: isHovered
              ? `linear-gradient(${105 + rotateY * 2}deg, ${theme === 'dark' ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.5)'} 0%, transparent 50%)`
              : 'none',
        ...
        <div
          ...
          style={{
            boxShadow: isHovered
              ? `inset 0 0 0 1px ${theme === 'dark' ? 'rgba(212,175,55,0.2)' : 'rgba(212,175,55,0.3)'}`
              : 'none',
            opacity: isHovered ? 1 : 0
          }}
        />
        ...
        <AnimatePresence>
          {kpi.tooltip && hoveredKPI === kpi.label && (
            ...
            >
              <div
                className={`px-3 py-2 rounded-lg max-w-[180px] ${theme === 'dark'
                  ? 'bg-[#1a1a1a] border border-white/10'
                  : 'bg-white border border-slate-200 shadow-lg'
                  }`}
              >
                <p className={`text-[11px] leading-relaxed ${theme === 'dark' ? 'text-white/80' : 'text-slate-600'}`}>
                  {kpi.tooltip}
                </p>
                <div className="absolute top-full left-1/2 transform -translate-x-1/2 -mt-[1px]">
                  <div
                    className={`w-2 h-2 rotate-45 ${theme === 'dark'
        ...
        <div className="flex flex-col justify-between h-full w-[60%] z-10 relative py-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className={`text-[11px] font-semibold whitespace-nowrap overflow-hidden text-ellipsis ${theme === 'dark' ? 'text-white/60' : 'text-slate-500'}`}>
              {kpi.label}
            </span>
          </div>
          ...
          {kpi.desc && !compact && (
            <p className="text-[10px] text-text-muted leading-normal mt-2 pr-2">
              {kpi.desc}
            </p>
          )}
        </div>
